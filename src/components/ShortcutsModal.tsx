...
export default function ShortcutsModal({ isDarkMode, onClose, onAddShortcut, editingShortcut }: ShortcutsModalProps) {
  ...
  return (
    ...
    >
      <div className={`bg-gray-800 rounded-xl p-6 w-11/12 max-w-lg shadow-2xl transition-all duration-300 animate-modalEntry ${isDarkMode ? 'bg-gray-800 text-white' : 'bg-white text-black'}`}
        onClick={e => e.stopPropagation()}>
        <div className="flex justify-between items-center mb-6 border-b border-gray-700 pb-4">
          <h2 className="text-xl font-bold text-white tracking-wide">{editingShortcut ? 'Edit Shortcut' : 'Add New Shortcut'}</h2>
          <button 
            onClick={onClose}
